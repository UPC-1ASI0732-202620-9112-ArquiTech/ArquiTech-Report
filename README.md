...
| HU04     | Supervisor de obra           | Alta     | EP01 | Consultar historial de movimientos de materiales                   | Como supervisor de obra, deseo consultar las entradas y salidas de materiales para realizar seguimiento de los movimientos del inventario.                                                                                      | AC1: Dado que existen movimientos asociados al proyecto, cuando el supervisor consulta el historial, entonces el sistema devuelve las entradas y salidas registradas con su información correspondiente.<br><br>AC2: Dado que el proyecto no posee movimientos registrados, cuando el supervisor consulta el historial, entonces el sistema devuelve un resultado vacío.                                                                                                             |
| HU05     | Supervisor de obra           | Alta     | EP01 | Registrar maquinaria                                               | Como supervisor de obra, deseo registrar maquinaria asociada a una obra para mantener control sobre los recursos utilizados en el proyecto.                                                                                     | AC1: Dado que el proyecto existe, cuando el supervisor registra una maquinaria con información válida, entonces el sistema almacena la maquinaria asociada al proyecto.<br><br>AC2: Dado que la información requerida de la maquinaria es inválida o incompleta, cuando se solicita su registro, entonces el sistema rechaza la operación y no crea el recurso.                                                                                                                      |
| HU28     | Supervisor de obra           | Alta     | EP01 | Consultar materiales                                               | Como supervisor de obra, deseo consultar los materiales registrados en una obra para conocer su disponibilidad e información asociada.                                                                                          | AC1: Dado que existen materiales registrados, cuando el supervisor consulta los materiales de una obra, entonces el sistema devuelve la información correspondiente.<br><br>AC2: Dado que no existen materiales asociados al proyecto, cuando el supervisor realiza la consulta, entonces el sistema devuelve un resultado vacío.                                                                                                                                                    |
...
| EP02     | —                            | Alta     | —    | Gestión de trabajadores, tareas y asistencia                       | Agrupa las funcionalidades relacionadas con trabajadores, asistencia del personal y actividades asignadas durante la ejecución de los proyectos.                                                                                | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| HU07     | Supervisor de obra           | Alta     | EP02 | Asignar tareas a trabajadores                                      | Como supervisor de obra, deseo asignar tareas a los trabajadores para organizar las actividades necesarias durante la ejecución de la obra.                                                                                     | AC1: Dado que el trabajador está registrado, cuando el supervisor crea una tarea válida asociada al trabajador, entonces el sistema registra la tarea y su asignación.<br><br>AC2: Dado que el trabajador no existe o la información requerida de la tarea es inválida, cuando se intenta realizar la asignación, entonces el sistema rechaza la operación.                                                                                                                          |
| HU08     | Supervisor de obra           | Alta     | EP02 | Consultar tareas                                                   | Como supervisor de obra, deseo consultar las tareas registradas para conocer sus responsables, fechas y estado de ejecución.                                                                                                    | AC1: Dado que existen tareas registradas, cuando el supervisor consulta las tareas, entonces el sistema devuelve la información correspondiente a cada actividad.<br><br>AC2: Dado que no existen tareas asociadas al proyecto, cuando se realiza la consulta, entonces el sistema devuelve un resultado vacío.                                                                                                                                                                      |
...
| HU54     | Supervisor de obra           | Alta     | EP02 | Registrar asistencia de trabajadores                               | Como supervisor de obra, deseo registrar la asistencia de los trabajadores para mantener actualizada la disponibilidad del personal durante la ejecución de la obra.                                                            | AC1: Dado que el trabajador se encuentra asociado a un proyecto, cuando el supervisor registra su asistencia para una fecha válida, entonces el sistema almacena el estado de asistencia asociado al trabajador y a la fecha correspondiente.<br><br>AC2: Dado que el trabajador no pertenece al proyecto o la información de asistencia es inválida, cuando el supervisor intenta registrar la asistencia, entonces el sistema rechaza la operación y no crea el registro.          |
| HU55     | Supervisor de obra           | Alta     | EP02 | Consultar asistencia de trabajadores                               | Como supervisor de obra, deseo consultar la asistencia registrada para conocer la disponibilidad del personal de una obra.                                                                                                      | AC1: Dado que existen registros de asistencia para el proyecto y periodo consultados, cuando el supervisor realiza la consulta, entonces el sistema devuelve los trabajadores, fechas y estados de asistencia correspondientes.<br><br>AC2: Dado que no existen registros para el periodo consultado, cuando el supervisor realiza la consulta, entonces el sistema devuelve un resultado vacío.                                                                                     |
| HU56     | Contratante                  | Alta     | EP02 | Consultar asistencia de una obra                                   | Como contratante, deseo consultar la asistencia registrada en una obra para conocer la disponibilidad del personal asociado al proyecto.                                                                                        | AC1: Dado que el contratante posee acceso al proyecto, cuando consulta la asistencia registrada, entonces el sistema devuelve la información de asistencia correspondiente a los trabajadores de dicha obra.<br><br>AC2: Dado que el contratante no posee acceso al proyecto, cuando solicita consultar su información de asistencia, entonces el sistema impide el acceso a los registros.                                                                                          |
| EP03     | —                            | Alta     | —    | Gestión y seguimiento de proyectos                                 | Agrupa las funcionalidades necesarias para registrar y consultar proyectos de construcción, sus responsables y el avance general de las obras.                                                                                  | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| HU09     | Supervisor de obra           | Alta     | EP03 | Registrar proyecto de construcción                                 | Como supervisor de obra, deseo registrar un proyecto para gestionar la información correspondiente a una nueva obra.                                                                                                            | AC1: Dado que los responsables y la información del proyecto son válidos, cuando el supervisor registra el proyecto, entonces el sistema almacena la nueva obra con sus datos correspondientes.<br><br>AC2: Dado que la información obligatoria es inválida o incompleta, cuando se intenta registrar el proyecto, entonces el sistema rechaza la operación y no crea la obra.                                                                                                       |
| HU22     | Supervisor de obra           | Alta     | EP03 | Consultar proyectos bajo supervisión                               | Como supervisor de obra, deseo consultar los proyectos asociados a mi gestión para realizar seguimiento de las obras bajo mi responsabilidad.                                                                                   | AC1: Dado que existen proyectos asociados al supervisor, cuando este consulta sus proyectos, entonces el sistema devuelve las obras correspondientes.<br><br>AC2: Dado que no existen proyectos asociados al supervisor, cuando realiza la consulta, entonces el sistema devuelve un resultado sin proyectos disponibles.                                                                                                                                                            |
| HU33     | Contratante                  | Alta     | EP03 | Consultar proyectos contratados                                    | Como contratante, deseo consultar los proyectos asociados a mi participación para realizar seguimiento de las obras contratadas.                                                                                                | AC1: Dado que existen proyectos asociados al contratante, cuando este consulta sus proyectos, entonces el sistema devuelve las obras correspondientes.<br><br>AC2: Dado que no existen proyectos asociados al contratante, cuando realiza la consulta, entonces el sistema devuelve un resultado sin proyectos disponibles.                                                                                                                                                          |
| HU57     | Supervisor de obra           | Alta     | EP03 | Actualizar avance de obra                                          | Como supervisor de obra, deseo actualizar el porcentaje de avance de un proyecto para mantener vigente el estado general de ejecución de la obra.                                                                               | AC1: Dado que el proyecto existe, cuando el supervisor registra un porcentaje de avance comprendido entre 0 y 100, entonces el sistema actualiza el avance asociado al proyecto.<br><br>AC2: Dado que el valor proporcionado se encuentra fuera del rango permitido o el proyecto no existe, cuando se solicita la actualización, entonces el sistema rechaza la operación y conserva el avance previamente registrado.                                                              |
| HU58     | Contratante                  | Alta     | EP03 | Consultar avance de obra                                           | Como contratante, deseo consultar el avance de un proyecto para conocer el estado general de ejecución de la obra contratada.                                                                                                   | AC1: Dado que el contratante posee acceso al proyecto, cuando consulta su información, entonces el sistema devuelve el porcentaje de avance registrado para la obra.<br><br>AC2: Dado que el contratante no posee acceso al proyecto, cuando solicita consultar su avance, entonces el sistema impide el acceso a la información.                                                                                                                                                    |
...
| HU11     | Visitante                    | Media    | EP04 | Conocer ArquiTech                                                  | Como visitante, deseo conocer la propuesta de ArquiTech para comprender el propósito de la solución y los problemas que busca resolver.                                                                                         | AC1: Dado que el visitante accede al contenido público de ArquiTech, cuando consulta la información de la solución, entonces el sistema presenta el propósito y la propuesta de valor de ArquiTech.<br><br>AC2: Dado que el visitante consulta la información general de la solución, cuando revisa su contenido, entonces el sistema identifica los segmentos a los que está dirigida y las principales capacidades ofrecidas para la gestión de obras.                             |
| HU12     | Visitante                    | Media    | EP04 | Consultar medios de contacto                                       | Como visitante, deseo conocer los medios de contacto de ArquiTech para disponer de canales mediante los cuales comunicarme con el equipo responsable.                                                                           | AC1: Dado que existen medios de contacto configurados para ArquiTech, cuando el visitante consulta la información de contacto, entonces el sistema proporciona los canales disponibles.<br><br>AC2: Dado que un medio de contacto se encuentra disponible, cuando el visitante utiliza la información proporcionada, entonces el sistema dispone de los datos necesarios para iniciar la comunicación mediante dicho canal.                                                          |
| HU13     | Visitante                    | Media    | EP04 | Explorar características y beneficios                              | Como visitante, deseo conocer las principales características y beneficios de ArquiTech para evaluar si la solución responde a mis necesidades.                                                                                 | AC1: Dado que el visitante desea conocer las capacidades de ArquiTech, cuando consulta la información del producto, entonces el sistema presenta sus principales características.<br><br>AC2: Dado que el visitante consulta los beneficios de la solución, cuando revisa la información disponible, entonces el sistema presenta los beneficios relacionados con la centralización y seguimiento de la información de las obras.                                                    |
| HU34     | Visitante                    | Media    | EP04 | Acceder a la plataforma desde la Landing Page                      | Como visitante, deseo acceder a la plataforma de ArquiTech desde el sitio público para comenzar a utilizar el producto digital.                                                                                                 | AC1: Dado que el visitante decide utilizar ArquiTech, cuando solicita acceder a la plataforma, entonces el sistema lo dirige al punto de acceso de la aplicación web.<br><br>AC2: Dado que se completa la navegación desde el sitio público hacia el producto digital, cuando el visitante accede a la aplicación web, entonces puede continuar con el proceso de autenticación.                                                                                                     |
...
| HU16     | Usuario registrado           | Media    | EP05 | Actualizar información del perfil                                  | Como usuario registrado, deseo actualizar la información de mi perfil para mantener mis datos personales configurados de acuerdo con mis preferencias.                                                                          | AC1: Dado que el usuario posee un perfil registrado, cuando proporciona nuevos datos válidos, entonces el sistema conserva la información actualizada.<br><br>AC2: Dado que la información del perfil ha sido actualizada, cuando el usuario vuelve a consultar su perfil, entonces el sistema recupera los datos previamente conservados.                                                                                                                                           |
| HU19     | Usuario registrado           | Media    | EP05 | Ajustar tamaño de fuente                                           | Como usuario registrado, deseo ajustar el tamaño de la fuente del producto para adaptar la lectura del contenido a mis necesidades.                                                                                             | AC1: Dado que el usuario utiliza el ajuste de aumento de fuente disponible, cuando aplica dicha preferencia, entonces el sistema incrementa el tamaño del texto mostrado durante su interacción.<br><br>AC2: Dado que el usuario ha modificado el tamaño de fuente, cuando solicita restablecer la configuración, entonces el sistema recupera el tamaño de texto predeterminado.                                                                                                    |
| HU46     | Usuario del producto digital | Media    | EP05 | Cambiar idioma del producto                                        | Como usuario del producto digital, deseo cambiar entre los idiomas disponibles para comprender el contenido de ArquiTech en el idioma de mi preferencia.                                                                        | AC1: Dado que ArquiTech dispone de contenido traducido al inglés y al español, cuando el usuario selecciona uno de estos idiomas, entonces el sistema presenta el contenido traducible en el idioma seleccionado.<br><br>AC2: Dado que el usuario ha seleccionado un idioma disponible, cuando continúa navegando por contenido que dispone de traducción durante la misma interacción, entonces el sistema utiliza el idioma seleccionado.                                          |
| EP06     | —                            | Alta     | —    | Incidencias y reportes                                             | Agrupa las funcionalidades relacionadas con el registro y seguimiento de incidencias y con la consulta y descarga de reportes de proyecto.                                                                                      | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| HU20     | Contratante                  | Alta     | EP06 | Consultar reporte semanal de una obra                              | Como contratante, deseo consultar un reporte semanal de una obra para conocer los principales acontecimientos y actividades registrados durante la semana.                                                                      | AC1: Dado que existen datos correspondientes al proyecto, cuando el contratante consulta el reporte semanal, entonces el sistema consolida la información principal del proyecto, las tareas completadas, las entradas y salidas de materiales y las incidencias registradas.<br><br>AC2: Dado que alguna categoría del reporte no posee registros, cuando se genera la información semanal, entonces el sistema presenta los datos disponibles sin impedir la consulta del reporte. |
...
| HU36     | Supervisor de obra           | Alta     | EP06 | Actualizar incidencia                                              | Como supervisor de obra, deseo actualizar una incidencia para mantener vigente su información, severidad y estado.                                                                                                              | AC1: Dado que la incidencia existe, cuando el supervisor proporciona información válida actualizada, entonces el sistema modifica los datos de la incidencia.<br><br>AC2: Dado que la incidencia no existe o los datos proporcionados son inválidos, cuando se solicita la actualización, entonces el sistema rechaza la operación y conserva la información anterior.                                                                                                               |
| HU37     | Supervisor de obra           | Alta     | EP06 | Consultar incidencias                                              | Como supervisor de obra, deseo consultar las incidencias registradas para realizar seguimiento de los problemas ocurridos en una obra.                                                                                          | AC1: Dado que existen incidencias registradas, cuando el supervisor las consulta, entonces el sistema devuelve su información, severidad y estado.<br><br>AC2: Dado que no existen incidencias registradas para el proyecto, cuando se realiza la consulta, entonces el sistema devuelve un resultado vacío.                                                                                                                                                                         |
| HU39     | Contratante                  | Alta     | EP06 | Consultar incidencias de una obra                                  | Como contratante, deseo consultar las incidencias de una obra para conocer los problemas registrados durante la ejecución del proyecto.                                                                                         | AC1: Dado que el contratante posee acceso al proyecto y existen incidencias asociadas, cuando las consulta, entonces el sistema devuelve la información disponible.<br><br>AC2: Dado que no existen incidencias registradas para el proyecto, cuando el contratante realiza la consulta, entonces el sistema devuelve un resultado vacío.                                                                                                                                            |
| HU51     | Supervisor de obra           | Alta     | EP06 | Eliminar incidencia                                                | Como supervisor de obra, deseo eliminar una incidencia registrada incorrectamente para mantener consistente la información del proyecto.                                                                                        | AC1: Dado que la incidencia existe, cuando el supervisor solicita su eliminación, entonces el sistema elimina el registro correspondiente.<br><br>AC2: Dado que la incidencia no existe, cuando se solicita su eliminación, entonces el sistema no modifica las incidencias registradas.                                                                                                                                                                                             |
| HU52     | Contratante                  | Alta     | EP06 | Descargar reporte semanal                                          | Como contratante, deseo descargar el reporte semanal de una obra para conservar una copia de la información consolidada del proyecto.                                                                                           | AC1: Dado que el reporte semanal dispone de información del proyecto, cuando el contratante solicita su descarga, entonces el sistema genera un documento con el contenido consolidado del reporte.<br><br>AC2: Dado que la generación del documento concluye correctamente, cuando el reporte es producido, entonces el sistema proporciona un archivo en formato PDF para su conservación.                                                                                         |
...
| TS03     | Developer                    | Alta     | EP01 | Validar disponibilidad de stock                                    | Como Developer, deseo validar la disponibilidad de materiales antes de procesar una salida para mantener la consistencia del inventario.                                                                                        | AC1: Dado que la cantidad solicitada es menor o igual al stock disponible, cuando se procesa la validación, entonces el servicio permite continuar con la salida.<br><br>AC2: Dado que la cantidad solicitada supera el stock disponible, cuando se ejecuta la validación, entonces el servicio rechaza la operación antes de modificar los datos.                                                                                                                                   |
...
| TS05     | Developer                    | Alta     | EP01 | Implementar registro de maquinaria                                 | Como Developer, deseo disponer de un servicio REST para registrar maquinaria asociada a proyectos y soportar su gestión desde el producto.                                                                                      | AC1: Dado que la solicitud contiene información válida, cuando el servicio procesa el registro, entonces crea la maquinaria y devuelve el recurso creado.<br><br>AC2: Dado que la solicitud contiene información inválida, cuando se procesa el registro, entonces el servicio devuelve una respuesta de error y no crea la maquinaria.                                                                                                                                              |
| TS24     | Developer                    | Alta     | EP01 | Implementar consulta de materiales                                 | Como Developer, deseo disponer de servicios REST para recuperar los materiales registrados y soportar su consulta desde las funcionalidades del producto.                                                                       | AC1: Dado que existen materiales almacenados, cuando se realiza una solicitud de consulta válida, entonces el servicio devuelve los recursos correspondientes.<br><br>AC2: Dado que no existen materiales, cuando se realiza la consulta, entonces el servicio devuelve una colección vacía válida.                                                                                                                                                                                  |
| TS25     | Developer                    | Alta     | EP01 | Implementar actualización de materiales                            | Como Developer, deseo disponer de un servicio REST para actualizar materiales existentes y mantener vigente la información del inventario.                                                                                      | AC1: Dado que el material existe y la solicitud contiene datos válidos, cuando el servicio procesa la actualización, entonces modifica el recurso y devuelve su nueva información.<br><br>AC2: Dado que el material no existe o la solicitud es inválida, cuando se procesa la actualización, entonces el servicio devuelve una respuesta de error y no altera el recurso.                                                                                                           |
| TS26     | Developer                    | Alta     | EP01 | Implementar consulta de maquinaria                                 | Como Developer, deseo disponer de servicios REST para recuperar la maquinaria registrada y soportar su consulta desde los proyectos.                                                                                            | AC1: Dado que existen recursos de maquinaria, cuando se realiza una solicitud válida, entonces el servicio devuelve los recursos correspondientes.<br><br>AC2: Dado que un identificador solicitado no corresponde a una maquinaria existente, cuando se realiza la consulta individual, entonces el servicio devuelve una respuesta indicando que el recurso no existe.                                                                                                             |
| TS27     | Developer                    | Alta     | EP01 | Implementar actualización de maquinaria                            | Como Developer, deseo disponer de un servicio REST para actualizar la información de maquinaria existente y mantener vigentes sus datos.                                                                                        | AC1: Dado que la maquinaria existe y los datos son válidos, cuando se procesa la solicitud de actualización, entonces el servicio modifica y devuelve el recurso actualizado.<br><br>AC2: Dado que la maquinaria no existe o los datos son inválidos, cuando se procesa la solicitud, entonces el servicio devuelve una respuesta de error sin alterar la información existente.                                                                                                     |
| TS34     | Developer                    | Alta     | EP01 | Implementar eliminación de materiales                              | Como Developer, deseo disponer de un servicio REST para eliminar materiales existentes y soportar la gestión del inventario.                                                                                                    | AC1: Dado que el material existe, cuando se procesa una solicitud válida de eliminación, entonces el servicio elimina el recurso y devuelve una respuesta satisfactoria.<br><br>AC2: Dado que el material no existe, cuando se solicita su eliminación, entonces el servicio devuelve una respuesta de error y no modifica otros recursos.                                                                                                                                           |
| TS35     | Developer                    | Alta     | EP01 | Implementar eliminación de maquinaria                              | Como Developer, deseo disponer de un servicio REST para eliminar registros de maquinaria existentes y mantener actualizados los recursos del proyecto.                                                                          | AC1: Dado que la maquinaria existe, cuando el servicio procesa una solicitud válida de eliminación, entonces elimina el recurso y confirma la operación.<br><br>AC2: Dado que la maquinaria no existe, cuando se solicita su eliminación, entonces el servicio devuelve una respuesta indicando que el recurso no se encuentra disponible.                                                                                                                                           |
| TS06     | Developer                    | Alta     | EP02 | Implementar registro de trabajadores                               | Como Developer, deseo disponer de un servicio REST para registrar trabajadores asociados a proyectos y soportar la gestión del personal.                                                                                        | AC1: Dado que la solicitud posee información válida de un trabajador, cuando el servicio procesa el registro, entonces crea y devuelve el trabajador correspondiente.<br><br>AC2: Dado que la información enviada es inválida, cuando se procesa la solicitud, entonces el servicio rechaza el registro y no crea el recurso.                                                                                                                                                        |
| TS07     | Developer                    | Alta     | EP02 | Implementar creación y asignación de tareas                        | Como Developer, deseo disponer de servicios REST para crear tareas y asociarlas al personal correspondiente para soportar la planificación de actividades.                                                                      | AC1: Dado que los recursos asociados son válidos, cuando se procesa una solicitud de creación de tarea, entonces el servicio almacena la tarea con su información y asignación correspondientes.<br><br>AC2: Dado que los datos de la tarea o sus referencias no son válidos, cuando se procesa la solicitud, entonces el servicio rechaza la operación y no crea la tarea.                                                                                                          |
| TS08     | Developer                    | Alta     | EP02 | Implementar consulta de tareas                                     | Como Developer, deseo disponer de servicios REST para recuperar las tareas registradas y soportar el seguimiento de sus actividades.                                                                                            | AC1: Dado que existen tareas registradas, cuando se realiza una solicitud válida, entonces el servicio devuelve las tareas correspondientes.<br><br>AC2: Dado que no existen tareas para el criterio solicitado, cuando se realiza la consulta, entonces el servicio devuelve una colección vacía válida.                                                                                                                                                                            |
| TS10     | Developer                    | Alta     | EP02 | Implementar consulta de trabajadores                               | Como Developer, deseo disponer de servicios REST para recuperar trabajadores y soportar su consulta y asociación con proyectos.                                                                                                 | AC1: Dado que existen trabajadores registrados, cuando se realiza una solicitud válida, entonces el servicio devuelve la información correspondiente.<br><br>AC2: Dado que el trabajador solicitado no existe, cuando se realiza una consulta individual, entonces el servicio devuelve una respuesta indicando que el recurso no se encuentra disponible.                                                                                                                           |
| TS28     | Developer                    | Alta     | EP02 | Implementar actualización de trabajadores                          | Como Developer, deseo disponer de un servicio REST para modificar información de trabajadores existentes y mantener vigentes sus datos.                                                                                         | AC1: Dado que el trabajador existe y los datos son válidos, cuando el servicio procesa la actualización, entonces modifica y devuelve el recurso actualizado.<br><br>AC2: Dado que el trabajador no existe o la información es inválida, cuando se procesa la solicitud, entonces el servicio devuelve una respuesta de error y conserva la información anterior.                                                                                                                    |
| TS29     | Developer                    | Alta     | EP02 | Implementar actualización de tareas                                | Como Developer, deseo disponer de un servicio REST para actualizar los datos y estado de una tarea y soportar su seguimiento.                                                                                                   | AC1: Dado que la tarea existe, cuando se procesa una actualización válida, entonces el servicio modifica y devuelve la información actualizada.<br><br>AC2: Dado que la tarea no existe o los datos son inválidos, cuando se solicita la actualización, entonces el servicio devuelve una respuesta de error y conserva la información anterior.                                                                                                                                     |
| TS36     | Developer                    | Alta     | EP02 | Implementar eliminación de trabajadores                            | Como Developer, deseo disponer de un servicio REST para eliminar trabajadores existentes y soportar la actualización del personal registrado.                                                                                   | AC1: Dado que el trabajador existe, cuando se procesa una solicitud válida de eliminación, entonces el servicio elimina el recurso y confirma la operación.<br><br>AC2: Dado que el trabajador no existe, cuando se solicita su eliminación, entonces el servicio devuelve una respuesta de error sin modificar otros trabajadores.                                                                                                                                                  |
| TS37     | Developer                    | Alta     | EP02 | Implementar eliminación de tareas                                  | Como Developer, deseo disponer de un servicio REST para eliminar tareas existentes y mantener actualizadas las actividades del proyecto.                                                                                        | AC1: Dado que la tarea existe, cuando se procesa una solicitud válida de eliminación, entonces el servicio elimina la tarea y confirma la operación.<br><br>AC2: Dado que la tarea no existe, cuando se procesa la solicitud, entonces el servicio devuelve una respuesta indicando que el recurso solicitado no se encuentra disponible.                                                                                                                                            |
| TS40     | Developer                    | Alta     | EP02 | Implementar registro de asistencia                                 | Como Developer, deseo disponer de un servicio REST para registrar la asistencia de trabajadores y soportar el control del personal de una obra.                                                                                 | AC1: Dado que la solicitud contiene un trabajador asociado al proyecto, una fecha válida y un estado de asistencia válido, cuando el servicio procesa el registro, entonces almacena la asistencia y devuelve el recurso creado.<br><br>AC2: Dado que el trabajador no pertenece al proyecto o la solicitud contiene información inválida, cuando el servicio la procesa, entonces rechaza la operación y no crea el registro de asistencia.                                         |
| TS41     | Developer                    | Alta     | EP02 | Implementar consulta de asistencia                                 | Como Developer, deseo disponer de servicios REST para recuperar registros de asistencia y soportar su consulta por proyecto y periodo.                                                                                          | AC1: Dado que existen registros de asistencia para los criterios solicitados, cuando se procesa una solicitud de consulta válida, entonces el servicio devuelve los registros correspondientes.<br><br>AC2: Dado que no existen registros para los criterios solicitados, cuando se realiza la consulta, entonces el servicio devuelve una colección vacía válida.                                                                                                                   |
| TS09     | Developer                    | Alta     | EP03 | Implementar registro de proyectos                                  | Como Developer, deseo disponer de un servicio REST para registrar proyectos de construcción y soportar su gestión en ArquiTech.                                                                                                 | AC1: Dado que la solicitud contiene información válida del proyecto y sus responsables, cuando el servicio procesa el registro, entonces crea el proyecto y devuelve el recurso creado.<br><br>AC2: Dado que la información requerida es inválida, cuando se procesa la solicitud, entonces el servicio rechaza la operación y no crea el proyecto.                                                                                                                                  |
| TS13     | Developer                    | Alta     | EP03 | Implementar consulta de proyectos                                  | Como Developer, deseo disponer de servicios REST para recuperar proyectos registrados y permitir su consulta según el usuario correspondiente.                                                                                  | AC1: Dado que existen proyectos compatibles con el criterio y autorización solicitados, cuando se realiza la consulta, entonces el servicio devuelve los proyectos correspondientes.<br><br>AC2: Dado que no existen proyectos compatibles, cuando se realiza la consulta, entonces el servicio devuelve una colección vacía válida.                                                                                                                                                 |
| TS42     | Developer                    | Alta     | EP03 | Implementar actualización de avance de proyecto                    | Como Developer, deseo disponer de un servicio REST para actualizar el porcentaje de avance de los proyectos y mantener vigente su estado de ejecución.                                                                          | AC1: Dado que el proyecto existe y la solicitud contiene un porcentaje entre 0 y 100, cuando el servicio procesa la actualización, entonces modifica el avance del proyecto y devuelve la información actualizada.<br><br>AC2: Dado que el proyecto no existe o el porcentaje se encuentra fuera del rango permitido, cuando se procesa la solicitud, entonces el servicio devuelve una respuesta de error y conserva el avance anterior.                                            |
| TS43     | Developer                    | Alta     | EP03 | Implementar consulta de avance de proyecto                         | Como Developer, deseo disponer de un servicio REST para recuperar el avance de los proyectos y soportar su seguimiento por usuarios autorizados.                                                                                | AC1: Dado que el proyecto existe y el usuario posee acceso autorizado, cuando el servicio procesa la consulta, entonces devuelve el porcentaje de avance correspondiente.<br><br>AC2: Dado que el usuario no posee acceso al proyecto, cuando solicita consultar su avance, entonces el servicio rechaza la solicitud.                                                                                                                                                               |
| TS11     | Developer                    | Alta     | EP06 | Implementar consulta de incidencias                                | Como Developer, deseo disponer de servicios REST para recuperar las incidencias registradas y permitir su seguimiento.                                                                                                          | AC1: Dado que existen incidencias almacenadas, cuando se realiza una solicitud de consulta válida, entonces el servicio devuelve los recursos correspondientes.<br><br>AC2: Dado que no existen incidencias para el criterio solicitado, cuando se realiza la consulta, entonces el servicio devuelve una colección vacía válida.                                                                                                                                                    |
| TS30     | Developer                    | Alta     | EP06 | Implementar registro de incidencias                                | Como Developer, deseo disponer de un servicio REST para registrar incidencias asociadas a proyectos y mantener evidencia de los problemas identificados.                                                                        | AC1: Dado que la solicitud contiene información válida, cuando el servicio procesa el registro, entonces crea la incidencia y devuelve el recurso correspondiente.<br><br>AC2: Dado que la información de la incidencia es inválida, cuando se procesa la solicitud, entonces el servicio devuelve una respuesta de error y no crea el recurso.                                                                                                                                      |
| TS31     | Developer                    | Alta     | EP06 | Implementar actualización de incidencias                           | Como Developer, deseo disponer de un servicio REST para actualizar la información y estado de incidencias existentes y soportar su seguimiento.                                                                                 | AC1: Dado que la incidencia existe y los datos son válidos, cuando el servicio procesa la actualización, entonces modifica y devuelve la incidencia actualizada.<br><br>AC2: Dado que la incidencia no existe o los datos son inválidos, cuando se solicita la actualización, entonces el servicio devuelve una respuesta de error y conserva la información anterior.                                                                                                               |
| TS38     | Developer                    | Alta     | EP06 | Implementar eliminación de incidencias                             | Como Developer, deseo disponer de un servicio REST para eliminar incidencias existentes y mantener consistente la información del proyecto.                                                                                     | AC1: Dado que la incidencia existe, cuando el servicio procesa una solicitud válida de eliminación, entonces elimina el recurso y confirma la operación.<br><br>AC2: Dado que la incidencia no existe, cuando se solicita su eliminación, entonces el servicio devuelve una respuesta de error sin modificar otros registros.                                                                                                                                                        |
...
| TS15     | Developer                    | Alta     | EP07 | Implementar autorización por roles en el backend                   | Como Developer, deseo aplicar permisos diferenciados en los servicios protegidos según el rol del usuario para impedir operaciones no autorizadas sobre los recursos.                                                           | AC1: Dado que una solicitud autenticada pertenece a un usuario cuyo rol posee autorización para la operación, cuando el backend evalúa el acceso al recurso, entonces permite procesar la solicitud.<br><br>AC2: Dado que el rol autenticado no posee autorización para la operación, cuando el backend evalúa la solicitud, entonces rechaza el acceso al recurso protegido.                                                                                                        |
...
| TS17     | Developer                    | Alta     | EP07 | Implementar gestión de sesión                                      | Como Developer, deseo gestionar el estado de autenticación en el cliente para mantener la sesión del usuario durante el uso autorizado de la aplicación.                                                                        | AC1: Dado que existe una autenticación válida, cuando el usuario continúa utilizando recursos protegidos, entonces el cliente conserva la información necesaria para identificar la sesión.<br><br>AC2: Dado que la sesión se finaliza, cuando se procesa el cierre, entonces el cliente elimina la información utilizada para identificarla.                                                                                                                                        |
| TS20     | Developer                    | Alta     | EP07 | Validar JWT en solicitudes protegidas                              | Como Developer, deseo validar los tokens enviados en solicitudes protegidas para rechazar accesos no autenticados o solicitudes con tokens inválidos.                                                                           | AC1: Dado que una solicitud contiene un token válido, cuando el backend procesa el acceso a un recurso protegido, entonces reconoce la autenticación de la solicitud y continúa con las validaciones de autorización correspondientes.<br><br>AC2: Dado que el token es inválido, inexistente o no puede validarse, cuando el backend procesa la solicitud, entonces rechaza el acceso al recurso protegido.                                                                         |
| TS21     | Developer                    | Alta     | EP07 | Proteger rutas del cliente según el rol                            | Como Developer, deseo controlar el acceso a las rutas protegidas del cliente web según el rol autenticado para evitar la navegación hacia funcionalidades no autorizadas.                                                       | AC1: Dado que el usuario posee el rol requerido para una ruta protegida, cuando intenta acceder a dicha ruta, entonces el cliente permite continuar con la navegación.<br><br>AC2: Dado que el usuario no posee el rol requerido, cuando intenta acceder a una ruta protegida, entonces el cliente impide continuar hacia dicho recurso.                                                                                                                                             |
| TS32     | Developer                    | Media    | EP07 | Implementar servicio de registro de usuarios                       | Como Developer, deseo disponer de un servicio REST de registro de usuarios para permitir la creación técnica de cuentas con la información correspondiente.                                                                     | AC1: Dado que una solicitud de registro contiene información válida y no duplicada, cuando el servicio la procesa, entonces crea el usuario y devuelve la información permitida del recurso creado.<br><br>AC2: Dado que la solicitud contiene información inválida o incompatible con una cuenta existente, cuando se procesa, entonces el servicio rechaza la creación y no genera una nueva cuenta.                                                                               |
| TS33     | Developer                    | Media    | EP07 | Implementar consulta de usuarios                                   | Como Developer, deseo disponer de servicios REST para recuperar información de usuarios requerida por otras funcionalidades del producto.                                                                                       | AC1: Dado que el usuario solicitado existe y la solicitud se encuentra autorizada, cuando se realiza la consulta, entonces el servicio devuelve la información permitida del usuario.<br><br>AC2: Dado que el usuario solicitado no existe, cuando se realiza la consulta, entonces el servicio devuelve una respuesta indicando que el recurso no se encuentra disponible.                                                                                                          |
| TS39     | Developer                    | Media    | EP08 | Documentar la API REST mediante OpenAPI                            | Como Developer, deseo disponer de documentación OpenAPI de los servicios REST para facilitar la comprensión y validación de los contratos expuestos por el backend.                                                             | AC1: Dado que los controladores REST se encuentran disponibles, cuando se genera la documentación de la API, entonces se describen los recursos, operaciones, parámetros y respuestas expuestos por el backend.<br><br>AC2: Dado que se modifica un contrato REST documentado, cuando se actualiza su definición, entonces la documentación refleja el contrato vigente.                                                                                                             |
| SP-01    | Developer                    | Alta     | EP08 | Analizar compatibilidad de contratos REST entre frontend y backend | Como Developer, deseo analizar los contratos utilizados por el frontend y los expuestos por el backend para determinar las incompatibilidades que deben resolverse antes de extender el producto.                               | AC1: Dado que existen servicios consumidos por el frontend y endpoints expuestos por el backend, cuando se comparan rutas, parámetros, payloads y respuestas, entonces se produce una matriz de compatibilidad con los hallazgos identificados.<br><br>AC2: Dado que la comparación ha finalizado, cuando se consolidan los hallazgos, entonces se documentan las incompatibilidades encontradas y las acciones técnicas recomendadas.                                               |
| SP-02    | Developer                    | Alta     | EP07 | Validar estrategia de autorización por roles                       | Como Developer, deseo validar el comportamiento de autenticación y autorización por roles para determinar si la estrategia existente protege correctamente los recursos antes de incorporar nuevas funcionalidades.             | AC1: Dado que existen los roles Supervisor y Contratante y recursos protegidos asociados, cuando se ejecutan pruebas de acceso permitido y denegado, entonces se documentan los resultados obtenidos.<br><br>AC2: Dado que las pruebas de autorización han finalizado, cuando se analizan sus resultados, entonces se documentan los riesgos encontrados y las recomendaciones técnicas correspondientes.                                                                            |
| SP-03    | Developer                    | Media    | EP05 | Evaluar persistencia de perfil y preferencias de accesibilidad     | Como Developer, deseo evaluar la estrategia utilizada para conservar la información de perfil y las preferencias de accesibilidad para determinar si resulta adecuada para la evolución del producto.                           | AC1: Dado que el producto conserva información de perfil y preferencias en el entorno del cliente, cuando se evalúan su persistencia, seguridad, alcance y recuperación, entonces se documentan los resultados de la evaluación.<br><br>AC2: Dado que la evaluación ha finalizado, cuando se analizan los resultados obtenidos, entonces se documenta una conclusión técnica y una recomendación para el siguiente incremento.                                                       |
| SP-04    | Developer                    | Alta     | EP08 | Validar despliegue integrado de los componentes de ArquiTech       | Como Developer, deseo validar la integración desplegada entre Landing Page, Frontend Web App y Backend Web App para determinar si los componentes reutilizados pueden utilizarse como línea base del nuevo ciclo de desarrollo. | AC1: Dado que existen los componentes desplegables del producto, cuando se ejecuta una prueba de navegación e integración de extremo a extremo, entonces se documenta el resultado de la prueba y las dependencias encontradas.<br><br>AC2: Dado que la validación integrada ha finalizado, cuando se consolidan sus resultados, entonces se documentan los problemas detectados y las acciones requeridas antes de continuar con nuevas funcionalidades.                            |
...
<p align="center">
  <img src="assets/chapter-4/software-object-oriented-design/class-diagram.png"
       alt="Class Diagram de ArquiTech"
       width="1100">
</p>
